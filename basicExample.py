...
def multiUserExample(sampleText: str):
    ...
    try:
        ...
        if 0 <= choice < len(allUserIds):
            ...
            runAutomationWithUser(selectedUserId, sampleText)
        else:
...
def runAutomationWithUser(userId: str, sampleText: str):
    ...
    try:
        if client.connectToGradio():
            success = client.generateSpeechWithUser(userId, sampleText)
